...
destacado.innerHTML=autoDestacado
.map((auto)=> `
        
<div class="card mx-5 text-info bg-dark" style="width: 18rem;">
<p class="card-text text-aling-center col-md-12">Destacado</p>
...
: destacado.innerHTML='No Existen autos Destacados'
...
const inputSearchChange = (event) => {
  ...
  filtroAuto.length > 0 ?
    divCards.innerHTML = filtroAuto
      .map(
        (auto) => `
                <div class="card mx-5 rounded card-img-top img-fluid img-fluid " style="width: 18rem;">
                <img src="${auto.foto}" class="card-img-top" alt="...">
                <div class="card-body">
                  <h5 class="card-title">${auto.marca}  ${auto.modelo}</h5>
                  <p class="card-text">Precio:$ ${auto.precio}</p>
